...
// PDF 섹션 타입
export type SectionType = 'header' | 'list' | 'paragraph' | 'definition' | 'table' | 'unknown'
...
export interface PDFLine {
  ...
  pageNum: number
  lineNum: number
  yPosition?: number // Y 좌표 (레이아웃용)
  fontSize?: number  // 폰트 크기
  fontName?: string  // 폰트 이름
  sectionType?: SectionType
  confidence?: number // 섹션 타입 신뢰도
}
...
export class HybridPDFProcessor {
   ...
   * PDF 파싱 with 레이아웃 보존
   */
  private async parsePDFWithLayout(buffer: Buffer): Promise<PDFLine[]> {
    try {
   ...
   * 섹션 타입 감지 및 그룹화
   */
  private identifySections(pdfLines: PDFLine[]): Section[] {
    const sections: Section[] = []
    let currentSection: Section | null = null
    
    pdfLines.forEach((line, idx) => {
      const sectionType = this.detectSectionType(
        line,
        idx > 0 ? pdfLines[idx - 1] : undefined,
        idx < pdfLines.length - 1 ? pdfLines[idx + 1] : undefined
      )
   ...
   * 섹션별 최적화된 단어 추출
   */
  private async extractWordsFromSections(
    sections: Section[], 
    fileName?: string
    ...
    for (const section of sections) {
      ...
      let sectionWords: ExtractedWord[] = []
      
      switch (section.type) {
        case 'definition':
          sectionWords = this.extractFromDefinitionSection(section)
          break
        
        case 'list':
          sectionWords = this.extractFromListSection(section)
          break
        
        case 'table':
          sectionWords = this.extractFromTableSection(section)
          break
        
        case 'paragraph':
          sectionWords = this.extractFromParagraphSection(section)
          break
      ...
      if (sectionWords.length > 0) {
        console.log(`  ${section.type} section: extracted ${sectionWords.length} words`)
        extractedWords.push(...sectionWords)
      }
    }
    
    console.log('Section type distribution:', sectionCounts)
    console.log(`Total words extracted from sections: ${extractedWords.length}`)
   ...
   * 정의 섹션에서 추출
   */
  private extractFromDefinitionSection(section: Section): ExtractedWord[] {
   ...
   * 리스트 섹션에서 추출
   */
  private extractFromListSection(section: Section): ExtractedWord[] {
    const words: ExtractedWord[] = []
   ...
   */
  private extractFromTableSection(section: Section): ExtractedWord[] {
    const words: ExtractedWord[] = []
   ...
   * 일반 단락에서 추출
   */
  private extractFromParagraphSection(section: Section): ExtractedWord[] {
    const words: ExtractedWord[] = []
  ...
  private async enhanceWithAI(
    ...
    if (wordsNeedingDef.length > 0) {
      const enhanced = await this.generateStructuredDefinitions(
        wordsNeedingDef.slice(0, 50),
        testType
      )
   ...
   * 구조화된 정의 생성
   */
  private async generateStructuredDefinitions(
    words: ExtractedWord[],
    testType: string
  ): Promise<ExtractedWord[]> {
    if (!this.openai) return words
...
Focus on accuracy and test-relevance. Include Korean translations in parentheses.`
    ...
    try {
      ...
      return enhancedWords.map((w: any) => ({
        ...w,
        confidence: 1.0
      }))
    } catch (error) {
  ...
  private detectTestType(fileName?: string): string {
    if (!fileName) return 'vocabulary'
    
    const lowerName = fileName.toLowerCase()
    if (/toefl/i.test(lowerName)) return 'TOEFL'
    if (/sat/i.test(lowerName)) return 'SAT'
    if (/gre/i.test(lowerName)) return 'GRE'
    if (/ielts/i.test(lowerName)) return 'IELTS'
    if (/toeic/i.test(lowerName)) return 'TOEIC'
    if (/수능/i.test(lowerName)) return '수능'
   ...
   * 중복 제거 및 품질 검증
   */
  private validateAndDeduplicate(words: ExtractedWord[]): ExtractedWord[] {
    console.log(`Starting validation with ${words.length} words`)
    ...
    console.log(`After deduplication: ${uniqueWords.size} unique words`)
    
    // 최종 검증 - 정의가 있거나 동의어가 있는 단어만 포함
    const validWords = Array.from(uniqueWords.values())
      .filter(w => {
        const hasDefinition = w.definition && w.definition.length > 0
        const hasSynonyms = w.synonyms && w.synonyms.length > 0
        const isValid = hasDefinition || hasSynonyms
        
        if (!isValid) {
          console.log(`Filtering out word without definition/synonyms: ${w.word}`)
        }
    ...
    console.log(`After validation: ${validWords.length} valid words`)
    return validWords
  }
